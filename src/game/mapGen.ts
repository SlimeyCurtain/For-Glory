...
export const DESERT_WIDTH = 3; // narrow divider, not a wide dead zone eating the map
export const ZONE_WIDTH = (MAP_COLS - DESERT_WIDTH) / 2; // player-ownable columns on each side
export const DESERT_COLS = Array.from({ length: DESERT_WIDTH }, (_, i) => ZONE_WIDTH + i);
export const CASTLE_COL = 4; // pulled in from the map edge -- a hub, not a back wall
export const CASTLE_ROW = Math.floor(MAP_ROWS / 2);
export const P1_CASTLE: Offset = { col: CASTLE_COL, row: CASTLE_ROW };
export const P2_CASTLE: Offset = { col: MAP_COLS - 1 - CASTLE_COL, row: CASTLE_ROW };
...
// ---------- zone terrain generation (player 1's side; mirrored for player 2) ----------

const inZone = (o: Offset) => o.col >= 0 && o.col < ZONE_WIDTH && o.row >= 0 && o.row < MAP_ROWS;
...
function growBlob(rng: Rng, seed: Offset, targetSize: number, eligible: (o: Offset) => boolean): Offset[] {
  ...
  while (chosen.length < targetSize && frontier.length > 0) {
    ...
    const candidates = neighborsOf(tile).filter((n) => inZone(n) && eligible(n) && !chosenKeys.has(key(n)));
...
function generateZone(rng: Rng): Map<string, TerrainType> {
  const terrain = new Map<string, TerrainType>();
  const castle = P1_CASTLE;
  ...
  for (let i = 0; i < rangeCount; i++) {
    ...
    for (let attempt = 0; attempt < 40; attempt++) {
      const c = { col: randInt(rng, 0, ZONE_WIDTH - 1), row: randInt(rng, 0, MAP_ROWS - 1) };
    ...
    for (let step = 0; step < length; step++) {
      ...
      if (!inZone(next) || !farEnough(next, 2)) {
        let found = false;
        for (let t = 0; t < 6; t++) {
          dir = (dir + 1) % 6;
  ...
  for (let i = 0; i < forestBlobCount; i++) {
    for (let attempt = 0; attempt < 20; attempt++) {
      const seedTile = { col: randInt(rng, 0, ZONE_WIDTH - 1), row: randInt(rng, 0, MAP_ROWS - 1) };
  ...
  // --- a winding river from the top edge to the bottom edge of the zone ---
  const riverNotMountain = (o: Offset) => terrain.get(key(o)) !== 'mountains';
  const startCol = randInt(rng, 1, ZONE_WIDTH - 2);
  ...
  while (cur.row < MAP_ROWS - 1 && guard < 150) {
    guard++;
    const neighbors = neighborsOf(cur).filter((n) => inZone(n) && riverNotMountain(n) && !visited.has(key(n)));
  ...
  for (let i = 0; i < riverPath.length; i++) {
    ...
    if (i > 1 && i < riverPath.length - 2 && rng() < 0.35) {
      const bankOptions = neighborsOf(riverPath[i]).filter((n) => inZone(n) && riverNotMountain(n));
...
/** BFS reachability for a plain ground unit (no mountains, no water) across the whole map. */
function isConnected(fullTerrain: Map<string, TerrainType>): boolean {
  const start = key(P1_CASTLE);
  const goal = key(P2_CASTLE);
  const seen = new Set([start]);
  const queue: Offset[] = [P1_CASTLE];
...
function ensureConnectivity(fullTerrain: Map<string, TerrainType>) {
  let guard = 0;
  while (!isConnected(fullTerrain) && guard < 200) {
    guard++;
    // BFS to find the reachable frontier, then clear the nearest blocking obstacle to it
    const seen = new Set([key(P1_CASTLE)]);
    const queue: Offset[] = [P1_CASTLE];
    ...
    blockers.sort((a, b) => hexDistance(a, P1_CASTLE) - hexDistance(b, P1_CASTLE));
    fullTerrain.set(key(blockers[0]), 'plains');
  }
}

export function generateMap(seed: number = Math.floor(Math.random() * 2 ** 31)): TileMap {
  const rng = mulberry32(seed);
  const p1Zone = generateZone(rng);

  const full = new Map<string, TerrainType>();
  for (let row = 0; row < MAP_ROWS; row++) {
    for (let col = 0; col < MAP_COLS; col++) {
      const o = { col, row };
      if (DESERT_COLS.includes(col)) {
        full.set(key(o), 'desert');
      } else if (col < ZONE_WIDTH) {
        full.set(key(o), p1Zone.get(key(o)) ?? 'plains');
      } else {
        const mirrored = { col: MAP_COLS - 1 - col, row };
        full.set(key(o), p1Zone.get(key(mirrored)) ?? 'plains');
      }
    }
  }
  full.set(key(P1_CASTLE), 'castleGround');
  full.set(key(P2_CASTLE), 'castleGround');

  ensureConnectivity(full);
...
/** Home zone ownership used to decide where each player may construct buildings. */
export function ownerOfCol(col: number): 1 | 2 | 0 {
  if (DESERT_COLS.includes(col)) return 0;
  return col < DESERT_COLS[0] ? 1 : 2;
}
